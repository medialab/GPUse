...
d3.formatMonth = d => {
  d.setHours(24); // Handle date generated by d3 is shifted one month because of timezones
...
function uncompress(compressed, callback) {
  useWebWorker(`
    ...
    self.onmessage = async (evt) => {
      const file = evt.data;
      try {
        self.postMessage(pako.ungzip(file, {to: "string"}));
      } catch(e) {
        self.postMessage("");
      }
    };
...
new Vue({
  ...
  mounted: function() {
    ...
    d3.json("data/list.json").then(metas => {
      this.fullStart = new Date(metas.start);
    ...
    downloadData: function() {
      ...
      monthsToDo.forEach(month => {
        this.gpus.forEach(gpu => {
          ...
          .catch(error => {
            this.gpusToDo = this.gpusToDo.filter(x => x !== gpu.id + "/" + month);
            console.log(error);
          });
        });
      });
